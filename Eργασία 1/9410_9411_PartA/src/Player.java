	...
	do	{
		...
		for(int w=0; w<board.apples.length; w++) {
			if(pos == board.apples[w].getAppleTileId()) {
				System.out.println("The player "+ getName() + " just ate an apple.");
		...
		for(int k=0; k<board.snakes.length; k++) {	
			if(pos==board.snakes[k].getHeadId()) {
				check=true;
				System.out.println("A snake bit the player " + getName() + "...SSHSHSHHS");
				results[1]++;
		...
		for(int k=0; k<board.ladders.length; k++) {
			if(pos == board.ladders[k].getDownStepId()) {
				check=true;
				System.out.println("The player "+ getName() + " just climbed a ladder");
				results[2]++;
