	...
	public static void main(String[] args) {
		...
		do {
			...
			for(Map.Entry<Integer, Integer> entry : b.entrySet()) {
				if(players.get(entry.getKey()) instanceof MinMaxPlayer) {
					array[entry.getKey()] = ((MinMaxPlayer) players.get(entry.getKey())).getNextMove(b3, array[entry.getKey()][0], array[entry.getKey()][0]);
				}
				else {
		...
		//H συνάρτηση στόχου για τον minmax player
		...
		System.out.println("The statistics for the MinMax player are :");
		p2.statistics(); //Εκτυπώνει την στατιστικά του παίκτη min max player
		System.out.println();
